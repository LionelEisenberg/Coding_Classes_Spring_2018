...
void parallelFilterFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  ...
  for (int y=0; y<filter_len; y++) {
    /* for all elements in the data */
    for (int x=0; x<data_len; x++) {
      /* it the data element matches the c  filter */
      if (input_array[x] == filter_list[y]) {
        /* include it in the output */
        output_array[x] = input_array[x];
      }
...
/* Optimized version of the previous function using loop unrolling */
void unrolledParallelDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  /* Variables for timing */
  struct timeval ta, tb, tresult;
...
/* Optimized function using dynamic scheduling */
void dynamicParallelDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  /* Variables for timing */
  struct timeval ta, tb, tresult;
...
/* Optimized function using static scheduling */
void staticParallelDataFirst ( int data_len, unsigned int* input_array, unsigned int* output_array, int filter_len, unsigned int* filter_list )
{
  /* Variables for timing */
  struct timeval ta, tb, tresult;
...
//  input_array = (unsigned int*) posix_memalign ( DATA_LEN * sizeof(unsigned int), 4096);
  ...
  //  printf("%d,", threads);
  // unrolledParallelDataFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list );
  // memset ( output_array, 0, DATA_LEN );

 //  printf("%d,", threads);
 // dynamicParallelDataFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list );
 // memset ( output_array, 0, DATA_LEN );

   printf("%d,", threads);
  staticParallelDataFirst ( DATA_LEN, input_array, output_array, filter_len, filter_list );
  memset ( output_array, 0, DATA_LEN );
  }
  printf("\n");
}
